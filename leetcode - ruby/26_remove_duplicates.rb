...
def remove_duplicates(nums)
  ...
  while i < nums.length
    if nums[i] > nums[i - 1]
      ...
      # non-unique values get passed by.
      nums[unique_count] = nums[i]
    end
    i += 1
  end
  # If nums isn't an empty array, then the zeroth value is always
  # going to be unique, it's the ones that come after that may not be.
  # That said, it has not yet been added to the unique count
  # (as we began at index 1).
  unique_count + 1
end
